...
def parse_figure(res_list):
    ...
    grouped_list=[]
    grouped_list_new=[]
    for text in res_list_filtered:#filter invalid char, such * @ etc. 
    ...
    for text in structured_list: #make the disorder data to structured
        print(f"text:{text},count:{count}") #for debug
        if is_numeric(text):
            print(f"is_numeric:{text}")
            item_info['item_qty']=text
            addValue2Dict(item_info_new=item_info_new,text=text)
            ...
            if item_info_new.get('item_name'):
                grouped_list_new.append(item_info_new)
                item_info_new = {}
            item_info_new['item_name'] = text
        ...
        if count%group_size==0:
            ...
            if count==num:  #last row
                grouped_list.append(item_info)
        count=count+1
    print(f"grouped_list_new:{grouped_list_new}")      
...
def addValue2Dict(item_info_new,text):
    old_value = item_info_new.get('value')
    if old_value:
        item_info_new['value'] = old_value+" "+text
    else:
        item_info_new['value'] = text
